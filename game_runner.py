...
def run_rhc_with_rl_and_collect_frames_3d_1v2(
    cfg: Dict[str, Any],
    steps: int | None = None,
    turn_len: int | None = None
):
    """
    1 defender vs 2 attackers rollout runner (shared attacker policy), with Level-A UKF:
      - Separate UKFs per link, no fusion:
          ukf_1a : 1 -> 2a  (defender estimates attacker0)
          ukf_1b : 1 -> 2b  (defender estimates attacker1)
          ukf_a1 : 2a -> 1  (attacker0 estimates defender)
          ukf_b1 : 2b -> 1  (attacker1 estimates defender)

    Observation matches Env._obs for Na=2:
        [p1-center,
         pA0-center, pA1-center,
         (pA0-p1), (pA1-p1),
         v1,
         vA0, vA1]    -> dim = (2 + 3*Na)*D = 8D for D=3

    Attacker policy is shared: attacker1 acts on an ego-permuted obs.

    UKF control knobs (Level A):
      cfg["use_ukf"] : bool
      cfg["ukf"]["who"] in {"none","1->2","2->1","both"}  (default "both")
      cfg["ukf"]["every"] measurement cadence
      cfg["runner_def_obs_from"] in {"truth","ukf_each","ukf_primary"} (default "truth")
      cfg["runner_primary_attacker"] in {"closest","idx0"} (default "closest")

    Returns dict includes:
      - legacy keys exec2_xyz etc. for attacker0
      - new keys exec3_xyz etc. for attacker1
      - est histories for any enabled UKFs
    """
    # -------------------- basics & dims --------------------
    D = int(cfg.get("D", np.asarray(cfg["x0"]).shape[1] // 2))
    nx = 2 * D
    ...
    dyn_name = (cfg.get("dynamics") or "hcw").lower()
    Na = int(cfg.get("num_attackers", 2))
    if Na != 2:
        raise ValueError(f"run_rhc_with_rl_and_collect_frames_3d_1v2 expects num_attackers=2, got {Na}")
    ...
    x1  = x0[0, :nx].copy()              # defender
    x2a = x0[1, :nx].copy()              # attacker 0
    if x0.shape[0] >= 3:
        x2b = x0[2, :nx].copy()          # attacker 1
    else:
        x2b = x0[1, :nx].copy()
    ...
    def build_train_obs_1v2(x1_vec: np.ndarray, x2a_vec: np.ndarray, x2b_vec: np.ndarray) -> np.ndarray:
        p1 = x1_vec[:D]; v1 = x1_vec[D:2*D]
        pA0 = x2a_vec[:D]; vA0 = x2a_vec[D:2*D]
        pA1 = x2b_vec[:D]; vA1 = x2b_vec[D:2*D]
        obs = np.concatenate(
            [
                p1 - center,
                pA0 - center, pA1 - center,
    ...
    def permute_obs_numpy_for_attacker(obs: np.ndarray, idx: int) -> np.ndarray:
        if idx == 0:
            return obs
        p1c  = obs[0:D]
        pA0c = obs[D:2*D]
        ...
        return np.concatenate(
            [p1c, pA1c, pA0c, rel1, rel0, v1, vA1, vA0],
            dtype=np.float32
        )
    ...
    expected_obs = (2 + 3 * Na) * D  # 8D
    if (din_def != expected_obs) or (din_att != expected_obs):
        print(
            f"[warn] ckpt expects obs_dim (def,att)=({din_def},{din_att}) "
            f"but runner will feed {expected_obs}. Make sure checkpoints match 1v2 obs."
        )
    ...
    # -------------------- helper: choose primary attacker (for "ukf_primary") --------------------
    primary_mode = (cfg.get("runner_primary_attacker") or "closest").lower()
    if primary_mode not in ("closest", "idx0"):
        primary_mode = "closest"

    def pick_primary_attacker(x2a_true: np.ndarray, x2b_true: np.ndarray) -> int:
        if primary_mode == "idx0":
            return 0
        pa = x2a_true[:D]
        pb = x2b_true[:D]
        da = float(np.dot(pa - center, pa - center))
        db = float(np.dot(pb - center, pb - center))
    ...
    # -------------------- Level-A UKFs (HCW-only) --------------------
    use_ukf = bool(cfg.get("use_ukf", False))
    if use_ukf and dyn_name != "hcw":
        print("[warn] UKF in this runner is HCW-only; disabling use_ukf for non-HCW dynamics.")
        use_ukf = False

    ukf_1a = ukf_1b = None   # defender->attacker0/1
    ukf_a1 = ukf_b1 = None   # attacker0/1->defender
    est_hist = None

    # what defender obs uses (truth/ukf_each/ukf_primary)
    def_obs_from = (cfg.get("runner_def_obs_from") or "truth").lower()
    if def_obs_from not in ("truth", "ukf_each", "ukf_primary"):
        def_obs_from = "truth"
    ...
    if use_ukf:
        ...
        # supported who values for Level A
        if who not in ("none", "1->2", "2->1", "both"):
            who = "both"
        ...
        # init filters
        def _init_filter_from_target(x_target: np.ndarray) -> KF_CV:
            x6 = _x6_from_xD(x_target)
            return KF_CV(
                np.r_[x6[:3], x6[3:6]], P0.copy(), Q.copy(), Rm.copy(), dt,
                kind="ukf", dyn="hcw", hcw=hcw_params
            )

        # 1->2: defender estimates both attackers
        if who in ("1->2", "both"):
            ukf_1a = _init_filter_from_target(x2a)
            ukf_1b = _init_filter_from_target(x2b)

        # 2->1: attackers estimate defender
        if who in ("2->1", "both"):
            ukf_a1 = _init_filter_from_target(x1)
            ukf_b1 = _init_filter_from_target(x1)

        # history dict
        est_hist = {}

        if ukf_1a is not None:
            est_hist["est12a_xyz"] = [ukf_1a.x[:3].copy()]
            est_hist["meas12a_azel"] = [None]
        if ukf_1b is not None:
            est_hist["est12b_xyz"] = [ukf_1b.x[:3].copy()]
            est_hist["meas12b_azel"] = [None]
        if ukf_a1 is not None:
            est_hist["est2a1_xyz"] = [ukf_a1.x[:3].copy()]
            est_hist["meas2a1_azel"] = [None]
        if ukf_b1 is not None:
            est_hist["est2b1_xyz"] = [ukf_b1.x[:3].copy()]
            est_hist["meas2b1_azel"] = [None]
    ...
    exec_xyz2.append(_p3_vec(x2a))
    exec_xyz3.append(_p3_vec(x2b))

    exec_att1.append({"R": _identity_R(), "phi": 0.0}); phi_hist1.append(0.0)
    exec_att2.append({"R": _identity_R(), "phi": 0.0}); phi_hist2.append(0.0)
    exec_att3.append({"R": _identity_R(), "phi": 0.0}); phi_hist3.append(0.0)
    ...
    for k in range(steps):
        # ---------- Build truth obs ----------
        obs_truth = build_train_obs_1v2(x1, x2a, x2b)

        # ---------- Defender obs (optionally uses UKF estimates) ----------
        if use_ukf and est_hist is not None and def_obs_from != "truth":
            # get attacker estimates in D-state form (fallback to truth if filter missing)
            def _estD_from_ukf(ukf: KF_CV | None, x_true: np.ndarray) -> np.ndarray:
                if ukf is None:
                    return x_true
                return _xD_from_x6(np.r_[ukf.x[:3], ukf.x[3:6]]).astype(np.float32)

            x2a_estD = _estD_from_ukf(ukf_1a, x2a)
            x2b_estD = _estD_from_ukf(ukf_1b, x2b)

            if def_obs_from == "ukf_each":
                obs_def = build_train_obs_1v2(x1, x2a_estD, x2b_estD)

            elif def_obs_from == "ukf_primary":
                prim = pick_primary_attacker(x2a, x2b)
                if prim == 0:
                    obs_def = build_train_obs_1v2(x1, x2a_estD, x2b)
                else:
                    obs_def = build_train_obs_1v2(x1, x2a, x2b_estD)
        ...
        # ---------- Attacker obs (ego-permutation, truth) ----------
        obs_att0 = obs_truth
        obs_att1 = permute_obs_numpy_for_attacker(obs_truth, idx=1)

        # ---------- Actions ----------
        u1  = np.clip(np.asarray(pol.act_def_obs(obs_def, deterministic=deterministic), dtype=np.float32), -umax, +umax)
        u2a = np.clip(np.asarray(pol.act_att_obs(obs_att0, deterministic=deterministic), dtype=np.float32), -umax, +umax)
        u2b = np.clip(np.asarray(pol.act_att_obs(obs_att1, deterministic=deterministic), dtype=np.float32), -umax, +umax)
        ...
        if debug_actions and k < 3:
            print(f"[k={k:02d}] ||a_def||={np.linalg.norm(u1):.3g}, "
                  f"||a_att0||={np.linalg.norm(u2a):.3g}, "
                  f"||a_att1||={np.linalg.norm(u2b):.3g}, "
                  f"first obs entries={obs_def[:6]}")

        # ---------- Plant step ----------
        x1  = step_plant_single(x1,  u1,  k)
        x2a = step_plant_single(x2a, u2a, k)
        x2b = step_plant_single(x2b, u2b, k)

        # ---------- UKF steps (HCW-only) ----------
        if use_ukf and est_hist is not None:
            idx_step = len(exec_xyz1)
            take = (idx_step % meas_every) == 0
            ...
            # --- predict steps ---
            if ukf_1a is not None:
                ukf_1a.predict(dt, u=_to3_u(u2a), u_cov=None)
            if ukf_1b is not None:
                ukf_1b.predict(dt, u=_to3_u(u2b), u_cov=None)
            if ukf_a1 is not None:
                ukf_a1.predict(dt, u=_to3_u(u1), u_cov=None)
            if ukf_b1 is not None:
                ukf_b1.predict(dt, u=_to3_u(u1), u_cov=None)

            # --- measurement update helper ---
            def _measure_and_update(ukf: KF_CV, p_obs_3: np.ndarray, p_tgt_3: np.ndarray):
                d = p_tgt_3 - p_obs_3
                d /= (np.linalg.norm(d) + 1e-12)
                b_b = d  # identity body frame
                az = np.arctan2(b_b[1], b_b[0]) + np.random.randn() * meas_std_az
                el = np.arctan2(
                    b_b[2],
                    np.sqrt(max(b_b[0]**2 + b_b[1]**2, 1e-18))
                ...
                z = np.array([az, el], float)
                ukf.update(z, p_obs=p_obs_3, R_wb=_identity_R())
                return z

            if take:
                # 1->2a: defender observes attacker0
                if ukf_1a is not None:
                    p_obs = _to3_pos(x1)
                    p_tgt = _to3_pos(x2a)
                    z = _measure_and_update(ukf_1a, p_obs, p_tgt)
                    est_hist["meas12a_azel"].append((p_obs.copy(), z.copy()))
                elif "meas12a_azel" in est_hist:
                    est_hist["meas12a_azel"].append(None)

                # 1->2b: defender observes attacker1
                if ukf_1b is not None:
                    p_obs = _to3_pos(x1)
                    p_tgt = _to3_pos(x2b)
                    z = _measure_and_update(ukf_1b, p_obs, p_tgt)
                    est_hist["meas12b_azel"].append((p_obs.copy(), z.copy()))
                elif "meas12b_azel" in est_hist:
                    est_hist["meas12b_azel"].append(None)

                # 2a->1: attacker0 observes defender
                if ukf_a1 is not None:
                    p_obs = _to3_pos(x2a)
                    p_tgt = _to3_pos(x1)
                    z = _measure_and_update(ukf_a1, p_obs, p_tgt)
                    est_hist["meas2a1_azel"].append((p_obs.copy(), z.copy()))
                elif "meas2a1_azel" in est_hist:
                    est_hist["meas2a1_azel"].append(None)

                # 2b->1: attacker1 observes defender
                if ukf_b1 is not None:
                    p_obs = _to3_pos(x2b)
                    p_tgt = _to3_pos(x1)
                    z = _measure_and_update(ukf_b1, p_obs, p_tgt)
                    est_hist["meas2b1_azel"].append((p_obs.copy(), z.copy()))
                elif "meas2b1_azel" in est_hist:
                    est_hist["meas2b1_azel"].append(None)
            else:
                # keep meas arrays aligned
                if "meas12a_azel" in est_hist: est_hist["meas12a_azel"].append(None)
                if "meas12b_azel" in est_hist: est_hist["meas12b_azel"].append(None)
                if "meas2a1_azel" in est_hist: est_hist["meas2a1_azel"].append(None)
                if "meas2b1_azel" in est_hist: est_hist["meas2b1_azel"].append(None)

            # --- log estimates ---
            if ukf_1a is not None:
                est_hist["est12a_xyz"].append(ukf_1a.x[:3].copy())
            if ukf_1b is not None:
                est_hist["est12b_xyz"].append(ukf_1b.x[:3].copy())
            if ukf_a1 is not None:
                est_hist["est2a1_xyz"].append(ukf_a1.x[:3].copy())
            if ukf_b1 is not None:
                est_hist["est2b1_xyz"].append(ukf_b1.x[:3].copy())
        ...
        exec_att1.append({"R": I, "phi": 0.0})
        exec_att2.append({"R": I, "phi": 0.0})
        exec_att3.append({"R": I, "phi": 0.0})
        phi_hist1.append(0.0); phi_hist2.append(0.0); phi_hist3.append(0.0)
    ...
    out = {
        ...
        "u_cmd_all": [
            np.asarray(u_cmd_1, dtype=float),
            np.asarray(u_cmd_2, dtype=float),
            np.asarray(u_cmd_3, dtype=float),
        ],
        "u_cmd_norm_all": [
            np.asarray(u_cmd_norm_1, dtype=float),
            np.asarray(u_cmd_norm_2, dtype=float),
            np.asarray(u_cmd_norm_3, dtype=float),
        ],

        "runner_primary_attacker_mode": primary_mode,
        "runner_def_obs_from": def_obs_from,
        "ukf_level": "A",
    }
    if est_hist is not None:
        out.update(est_hist)
...
# cfg["use_ukf"] = True
# cfg["ukf"]["who"] = "both"
# cfg["runner_def_obs_from"] = "ukf_each"