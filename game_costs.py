...
def build_game_costs(
    ...
    elif k == "guard_center_v5":  # balloon-style stage-only costs
        ar = (cfg.get("arena") or {})
        c  = [float(ar.get(k, 0.0)) for k in (["cx","cy"] if D==2 else ["cx","cy","cz"])]

        # ---- Attacker (P2) weights/params (from slide) ----
        P_pen_att = float(cfg.get("P_penalty_att", 1.0))  # numerator of proximity term
        r2        = float(cfg.get("R2",            1e-2)) # effort
        # ---- Defender (P1) weights/params (from slide) ----
        P_pen_def = float(cfg.get("P_penalty_def", 1.0))  # defender's proximity weight
        D_rate    = float(cfg.get("D_rate",        2.0))  # exp warning length-scale
        Vmax_att  = float(cfg.get("Vmax_att",      1.0))  # scale for speed-over-distance term
        r1        = float(cfg.get("R1",            1e-3)) # effort
        # numerics
        eps = float(cfg.get("eps_cost", 1e-9))

        # ---- helpers (Pyomo-friendly) ----
        def _pos(x,k_,D):  return [x[k_,i] for i in range(D)]
        def _sumsq(v):     return sum(vi*vi for vi in v)
        def _norm(v):      return sqrt(_sumsq(v) + eps)
        def _to_center_sq(p):
            return _sumsq([p[i] - (c[i] if i < len(c) else 0.0) for i in range(D)])

        # ---------------- Stage costs ----------------
        # Attacker:  ||x2 - c||  +  P_pen_att/(eps + ||x2 - x1||^2)  +  r2||u2||^2
        def l2_k(m, k_):
            p1 = _pos(m.x1, k_, D); p2 = _pos(m.x2, k_, D)
            u2 = [m.u2[k_, j] for j in m.U]
            dist_center   = _to_center_sq(p2)                            # ||x2-c||^2
            prox_defender = P_pen_att / (eps + _sumsq([p2[i]-p1[i] for i in range(D)]))
            effort        = r2 * _sumsq(u2)
            return dist_center + prox_defender + effort

        # Defender (single attacker, so weight normalization = 1):
        #  -[ P_pen_def/(eps + ||x2 - x1||^2) + ||x2 - c||^2 ]
        #  + exp( - ||x2 - c|| / D_rate )
        #  + 5 * Vmax_att / (eps + ||x2 - c|| )
        #  + r1||u1||^2
        def l1_k(m, k_):
            p1 = _pos(m.x1, k_, D); p2 = _pos(m.x2, k_, D)
            u1 = [m.u1[k_, j] for j in m.U]
            d2c       = _norm([p2[i] - (c[i] if i < len(c) else 0.0) for i in range(D)])
            dist_center_sq = d2c*d2c
            prox_term   = P_pen_def / (eps + _sumsq([p2[i]-p1[i] for i in range(D)]))
            warn_term   = exp( - d2c / D_rate )
            speed_over_d= 5.0 * Vmax_att / (eps + d2c)
            effort      = r1 * _sumsq(u1)
            # negative sign in front of bracket (as in slide): encourage keeping attacker far & close to defender
            return -(prox_term + dist_center_sq) + warn_term + speed_over_d + effort

        # ---------------- Terminal costs ----------------
        # Slides used “sum of stages” → no extra terminal term
        def l1_T(m): return 0.0
        def l2_T(m): return 0.0
